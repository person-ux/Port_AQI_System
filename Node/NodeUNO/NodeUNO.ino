...
SoftwareSerial PMS5003_Serial(6,5); //unoRx=7, unoTx=6  Software Serial for PMS5003
DHT_Unified dht(DHTPIN, DHTTYPE); //make dht object

float LOC[2] = {0,0}; //[long,latt] //enter location of node
...
{ 
  ...
  float fPrevVal = 0;
  //////////////////////////////////////////////PMS5003//////////////////////////////////////////////////
  if(PMS5003_Serial.find(0x42)){         //start of the transmission
    PMS5003_Serial.readBytes(buff,LENG); // store 31 bytes of data in buff
  ...
  if((now_time >= next_upload)&&(now_time > 299999)){ next_upload = millis() + 300000;  //5min
//  if((now_time >= next_upload)&&(now_time > 59999)){next_upload = millis() + 60000;       //1min
    SendData();
  }
  else if(AlertSend){
    SendData();
